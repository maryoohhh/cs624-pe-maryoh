...
export default class App extends Component {
  ...
  addCurrency = (currency, country) => {
    const index2 = this.state.countries.findIndex(item => {
      return item.id === country.id
    })
    const chosenCountry = this.state.countries[index2]
    chosenCountry.currencies.push(currency)
    const countries = [
      ...this.state.countries.slice(0, index2),
      chosenCountry,
      ...this.state.countries.slice(index2 + 1)
    ]
    this.setState({
      countries
    })
